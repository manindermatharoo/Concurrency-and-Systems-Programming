...
typedef struct recv_multiple_chunks
{
    PNG_file_data_p png_img_chunk;                           /* memory to hold the png data after recieve buf */
    IDAT_uncompressed_data_p uncompressed_data_png_images;   /* memory to hold uncompressed png data */
    bool *checked_png_chunk;                                 /* check to see if the png image chunk has been already seen */
    int number_of_images_received;                           /* how many unique png images have been found */
    char url[256];                                           /* store the url that needs to be checked */
} *recv_multiple_chunks_p;
